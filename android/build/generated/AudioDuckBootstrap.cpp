...
#define TAG "android.duck.audio"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> AudioDuck_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("AudioDuck.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::AudioDuckBindings::lookupGeneratedInit(
...
static void AudioDuck_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(AudioDuck_getBinding)->GetFunction());
}

static void AudioDuck_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::AudioDuckBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AudioDuckBinding = {
	"android.duck.audio",
	AudioDuck_init,
	AudioDuck_dispose
};
...
Java_android_duck_audio_AudioDuckBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("android.duck.audio", &AudioDuckBinding);
	titanium::KrollBindings::addExternalLookup(&(::AudioDuckBindings::lookupGeneratedInit));
}
